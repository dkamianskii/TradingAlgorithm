...
from trading.trade_algorithms.predictive_trade_algorithms.arima_trade_algorithm import ARIMATradeAlgorithm, ARIMATradeAlgorithmHyperparam
...
for i in range(eb.random_grid_search_attempts - 2):
    ...
    arima_hyperparams_grid = ARIMATradeAlgorithm.get_default_hyperparameters_grid()
    if not use_atr:
        for arima_hyperparams in arima_hyperparams_grid:
            arima_hyperparams[ARIMATradeAlgorithmHyperparam.TAKE_ACTION_BARRIER] = bid_risk_rate * 0.75
            arima_hyperparams[ARIMATradeAlgorithmHyperparam.ACTIVE_ACTION_MULTIPLIER] = active_action_multiplier
    ...
    for company in eb.companies_names:
        manager.set_tracked_stock(company, eb.companies_data[company]["train data"], ARIMATradeAlgorithm(), arima_hyperparams_grid)
