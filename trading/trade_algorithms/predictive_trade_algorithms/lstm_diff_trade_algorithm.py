...
class LSTMDiffTradeAlgorithmHyperparam(BaseEnum):
    TEST_PERIOD_SIZE = 1
    PREDICTION_PERIOD = 2
...
class LSTMDiffTradeAlgorithm(AbstractTradeAlgorithm):
    name = "LSTM on price diff trade algorithm"
    model_directory = "../../../models/lstm_diff/"
    #model_directory = "../models/lstm_diff/"
    ...
    def get_algorithm_name() -> str:
        return LSTMDiffTradeAlgorithm.name
    ...
    def create_hyperparameters_dict(test_period_size: int = 60, prediction_period: int = 3,
        ...
        return {
            LSTMDiffTradeAlgorithmHyperparam.TEST_PERIOD_SIZE: test_period_size,
            LSTMDiffTradeAlgorithmHyperparam.PREDICTION_PERIOD: prediction_period,
            LSTMDiffTradeAlgorithmHyperparam.TAKE_ACTION_BARRIER: take_action_barrier,
            LSTMDiffTradeAlgorithmHyperparam.ACTIVE_ACTION_MULTIPLIER: active_action_multiplier
        }

    @staticmethod
    def get_default_hyperparameters_grid() -> List[Dict]:
        return [LSTMDiffTradeAlgorithm.create_hyperparameters_dict(),
                LSTMDiffTradeAlgorithm.create_hyperparameters_dict(prediction_period=5)]
    ...
    def __define_model_name(self):
        ...
        self._model_name = LSTMDiffTradeAlgorithm.model_directory + base_name
    ...
    def __create_train_dataset(self, scaled_data, closes_diff, window_size: int = 20) -> Tuple[np.ndarray, np.ndarray]:
        dataframe = []
        y = []
        for i in range(len(scaled_data) - (self._prediction_period + window_size - 1)):
            window = scaled_data[i:(i + window_size)]
            dataframe.append(window)
            predicted_diff = np.sum(closes_diff[(i + window_size):(i + window_size + self._prediction_period)])
            y.append(predicted_diff)
        dataframe = np.array(dataframe, dtype=np.float32)
        y = np.array(y, dtype=np.float32)
        return dataframe, y

    def __choose_best_model(self, scaled_data, closes_diff):
        model_grid = LSTMDiffTradeAlgorithm.model_grid
        ...
        for window_size in model_grid[ModelGridColumns.WINDOW_SIZE]:
            x, y = self.__create_train_dataset(scaled_data, closes_diff, window_size)
            x_train = x[:-self._test_period_size]
            y_train = y[:-self._test_period_size]
            for i in range(0, LSTMDiffTradeAlgorithm.random_grid_search_attempts):
                ...
                model.compile(optimizer=optimizer, loss=MeanSquaredError(),
                              metrics=["mse", "mae"])
                history = model.fit(x=x_train, y=y_train, epochs=LSTMDiffTradeAlgorithm.epochs, shuffle=False,
    ...
    def train(self, data: pd.DataFrame, hyperparameters: Dict):
        ...
        self.__clear_vars()
        self._prediction_period = hyperparameters[LSTMDiffTradeAlgorithmHyperparam.PREDICTION_PERIOD]
        self._test_period_size = hyperparameters[LSTMDiffTradeAlgorithmHyperparam.TEST_PERIOD_SIZE]
        self._take_action_barrier = hyperparameters[LSTMDiffTradeAlgorithmHyperparam.TAKE_ACTION_BARRIER]
        self._active_action_multiplier = hyperparameters[LSTMDiffTradeAlgorithmHyperparam.ACTIVE_ACTION_MULTIPLIER]
        ...
        self.diff_data = selected_data.diff(axis=0)[1:]
        closes_diff = self.data["Close"].diff()[1:]
        scaled_data = self._input_scaler.fit_transform(self.diff_data)
        ...
        if exists(self._model_name + ".h5"):
            ...
            self.__choose_best_model(scaled_data, closes_diff)
            self.__save_model()

        print("Model params")
        print(self._model_params)

        x, y = self.__create_train_dataset(scaled_data, closes_diff, self._model_params[ModelGridColumns.WINDOW_SIZE.name])
        x_test = x[-self._test_period_size:]
        target_test = self.data["Close"][-len(x_test):]
        ...
        for i in range(0, len(predictions)):
            ...
            abs_relative_diff = np.abs((predictions[i][0] + cur_close - target_test[i]) / cur_close)
        ...
        history = self._model.fit(x=x, y=y, epochs=LSTMDiffTradeAlgorithm.epochs, validation_split=0.05,
                                  shuffle=False, callbacks=[es], verbose=0)
        print(f"Best model MSE = {history.history['mse'][-1]}, Val MSE = {history.history['val_mse'][-1]}")

        predictions = self._model.predict(x=x, verbose=0).flatten()
        closes_before_prediction = self.data["Close"][self._model_params[ModelGridColumns.WINDOW_SIZE.name]:-self._prediction_period]
        predictions = predictions + closes_before_prediction
        ...
        for i in range(self._prediction_period - 1, -1, -1):
            ...
            self.predictions.append(prediction[0][0] + self.data["Close"][-(i + 1)])

    def __transform_point(self, index: int = 0) -> np.ndarray:
        window_size = self._model_params[ModelGridColumns.WINDOW_SIZE.name]
        if index == 0:
            window = self.diff_data[-window_size:]
        else:
            window = self.diff_data[-(window_size + index): -index]
    ...
    def evaluate_new_point(self, new_point: pd.Series, date: Union[str, pd.Timestamp],
        ...
        point_diff = new_point - self.data.iloc[-1]
        self.data.loc[date] = new_point
        point_diff = point_diff.drop("Adj Close")
        self.diff_data.loc[date] = point_diff
        last_close = new_point["Close"]
        final_action = TradeAction.NONE
        new_point_scaled = self.__transform_point()
        predicted_diff = self._model.predict(new_point_scaled, verbose=0)[0][0]
        self.predictions.append(predicted_diff + last_close)
        relative_diff = predicted_diff / last_close
    ...
    def plot(self, img_dir: str, start_date: Optional[pd.Timestamp] = None,
        ...
        intend = self._prediction_period + self._model_params[ModelGridColumns.WINDOW_SIZE.name]
