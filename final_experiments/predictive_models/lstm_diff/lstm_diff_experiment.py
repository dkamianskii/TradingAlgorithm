...
from trading.trade_algorithms.predictive_trade_algorithms.lstm_diff_trade_algorithm import LSTMDiffTradeAlgorithm, LSTMDiffTradeAlgorithmHyperparam
...
for i in range(eb.random_grid_search_attempts - 2):
    ...
    lstm_hyperparams_grid = LSTMDiffTradeAlgorithm.get_default_hyperparameters_grid()
    if not use_atr:
        for lstm_hyperparams in lstm_hyperparams_grid:
            lstm_hyperparams[LSTMDiffTradeAlgorithmHyperparam.TAKE_ACTION_BARRIER] = bid_risk_rate * 0.75
            lstm_hyperparams[LSTMDiffTradeAlgorithmHyperparam.ACTIVE_ACTION_MULTIPLIER] = active_action_multiplier
    ...
    for company in eb.companies_names:
        manager.set_tracked_stock(company, eb.companies_data[company]["train data"], LSTMDiffTradeAlgorithm(), lstm_hyperparams_grid)
